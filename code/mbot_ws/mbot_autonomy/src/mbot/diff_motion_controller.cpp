...
{
private:
    float Kx = 2.0;     // largest fwd_vel is 1.0
    float Kw = 4.0;
    float d_end_crit = 0.02;
    float d_end_midsteps = 0.08;
    float angle_end_crit = 0.2;
...
public:
    ...
    virtual mbot_lcm_msgs::twist2D_t get_command(const mbot_lcm_msgs::pose2D_t& pose, const mbot_lcm_msgs::pose2D_t& target) override
    {
        ...
        float alpha = angle_diff(atan2(dy,dx), pose.theta);
        float L = 1.0;
        float xL = L * cos(alpha);
        float yL = L * sin(alpha);
        float r = L * L / (2 * yL);

        float fwd_vel = vel_sign * Kx * dist;
        float turn_vel = Kw / r;
...
public: 
    ...
    */
    ...
    void handlePath(const lcm::ReceiveBuffer* buf, const std::string& channel, const mbot_lcm_msgs::path2D_t* path)
    {
        ...
        if(usingPurePursuit)        state_ = PURE_PURSUIT;
        else                        state_ = SMART;
    ...
    bool usingPurePursuit = true;       // default to using pure pursuit
...
private:
    ...
    enum State
    {
        ...
        SMART,          // use Smart controller (RTR)
        PURE_PURSUIT    // use Pure Pursuit controller
    };
...
int main(int argc, char** argv)
{
    ...
    if (argc > 1){      // default using pure pursuit if no argument is given
        if (strcmp(argv[1], "pure") == 0)
            controller.usingPurePursuit = true;
        else if (strcmp(argv[1], "smart") == 0) 
            controller.usingPurePursuit = false;
    }

    if (controller.usingPurePursuit)
        std::cout << "Using Pure Pursuit controller." << std::endl;
    else
        std::cout << "Using Smart controller." << std::endl;
