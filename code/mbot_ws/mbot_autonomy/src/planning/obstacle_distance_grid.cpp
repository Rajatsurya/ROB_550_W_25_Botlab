...
void expand_node(const DistanceNode& node, ObstacleDistanceGrid& grid, std::priority_queue<DistanceNode>& search_queue)
{
    ...
    for (int n = 0; n < 8; ++n)
    {
        ...
        if (grid.isCellInGrid(adjacentCell.x, adjacentCell.y))
        {
            ...
            // 1st: Not seen yet, distance -1 is used to indicate that we haven't initialized distance in the cell
            // 2nd: Seen but there is a shorter distance to the cell
            if ((grid(adjacentCell.x, adjacentCell.y) < 0) || (grid(adjacentCell.x, adjacentCell.y) > distance * grid.metersPerCell()))
            {
                ...
                grid(adjacentCell.x, adjacentCell.y) = adjacentNode.distance * grid.metersPerCell();    // set distance
